...
import { getAllUsers, getAllTemplates, getAllResolutions, updateUserStatus, updateUserPermission, deleteUserTemplate, deleteResolution } from '../services/storage';
import { User, UserTemplate, ResolutionData, UserRole } from '../types';
import { Users, FileText, LayoutTemplate, Shield, Check, X, Trash2, Power } from 'lucide-react';
...
const AdminPanel: React.FC = () => {
  ...
  const handleDeleteTemplate = (id: string) => {
    if (confirm('Permanently delete this template? It will no longer be available for use.')) {
  ...
  const handleDeleteResolution = (id: string) => {
    if (confirm('Permanently delete this resolution from master records?')) {
      deleteResolution(id);
      refresh();
    }
  };
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {resolutions.length === 0 && (
               <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500">No resolutions recorded.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="flex space-x-1 border-b border-slate-200 mb-6">
        <button 
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 transition-colors ${
             activeTab === 'users' ? 'bg-white border-x border-t border-slate-200 text-brand-600 shadow-[0_-1px_3px_rgba(0,0,0,0.05)]' : 'text-slate-500 hover:text-slate-700 bg-slate-50'
          }`}
        >
        ...
        <button 
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 transition-colors ${
             activeTab === 'templates' ? 'bg-white border-x border-t border-slate-200 text-brand-600 shadow-[0_-1px_3px_rgba(0,0,0,0.05)]' : 'text-slate-500 hover:text-slate-700 bg-slate-50'
          }`}
        >
        ...
        <button 
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 transition-colors ${
             activeTab === 'resolutions' ? 'bg-white border-x border-t border-slate-200 text-brand-600 shadow-[0_-1px_3px_rgba(0,0,0,0.05)]' : 'text-slate-500 hover:text-slate-700 bg-slate-50'
          }`}
        >
